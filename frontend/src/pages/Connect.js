...
function Connect({ setUserEmail }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        setUser(user);
        setUserEmail(user.email); // Set the user's email when they are authenticated
      }
    });
  ...
  const connectWallet = async () => {
    try {
      ...
      setUser(result.user);
      setUserEmail(result.user.email); // Set the user's email after successful sign-in
    } catch (error) {
  ...
  const disconnectWallet = async () => {
    try {
      await signOut(auth);
      setUser(null);
      setUserEmail(null); // Clear the user's email on sign out
    } catch (error) {
